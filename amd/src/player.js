...
define(['core/ajax', 'core/notification'], function(Ajax, Notification) {
    ...
    const youtubeAdapter = async(config) => {
        await loadScript(
            ...
            () => Boolean(window.YT && window.YT.Player)
        );
    ...
    const vimeoAdapter = async(config) => {
        ...
        player.on('timeupdate', (data) => {
            timeCallbacks.forEach((callback) => callback(
                data.seconds || 0,
                data.duration || 0,
                playing
            ));
    ...
    const init = async(config) => {
        ...
        const openCheckpoint = async(checkpoint) => {
            ...
            submitButton.textContent = isConfirm
        ...
        const firstPendingCrossedCheckpoint = (from, to) => {
            ...
            for (const checkpoint of checkpoints) {
                if (
                    completed.has(Number(checkpoint.id)) ||
                    triggered.has(Number(checkpoint.id))
                ) {
                    continue;
                }

                if (
                    checkpoint.seconds <= to + 0.25 &&
                    checkpoint.seconds >= from - 0.25
                ) {
                    return checkpoint;
                }
            }
        ...
        const handleTick = async(current, eventDuration) => {
            ...
            try {
                ...
                if (lastTime !== null) {
                    ...
                    const checkpoint = firstPendingCrossedCheckpoint(
                        Math.min(lastTime, current),
                        current
                    );
        ...
        submitButton.addEventListener('click', async() => {
            ...
            if (
                blockedCheckpoint.checkpointtype !== 'confirm' &&
                response === ''
            ) {
        ...
        adapter.onTime((current, eventDuration) => {
            handleTick(
                Number(current || 0),
                Number(eventDuration || duration || 0)
            ).catch(Notification.exception);
        });
